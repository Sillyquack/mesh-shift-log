...
export function buildFinancialLocalId({ date, shiftKey, signoffType, terminalId = '', invoiceReference = '', authUserId = '', eventId = '' }) {
  return [
    'financial',
    date || 'unknown-date',
...
function statusForRecord(record) {
  const hasIssue = ['invoiceSent', 'salesPunched', 'settlementPerformed'].some((field) => record[field] !== 'yes');
...
function recordToPayload(record, authUserId) {
  ...
  const localId = record.localId || buildFinancialLocalId({
    date: record.date,
  ...
  return {
    ...
    amount_expected: record.amountExpected === '' ? null : record.amountExpected,
...
export async function upsertFinancialSignoff(record) {
  ...
  if (!record?.date) return { ok: false, mode: 'validation_error', message: 'Missing signoff date.' };
  ...
  return { ok: true, mode: 'authenticated', record: normalizeFinancialSignoff(data), row: data };
}
...
export async function fetchFinancialSignoffsForDate(date) {
  ...
  return { ok: true, mode: 'authenticated', records: (data || []).map(normalizeFinancialSignoff).filter(Boolean), rows: data || [] };
}
...
function logicalKeys(record) {
  return [...new Set([record.localId, record.backendId, baseLogicalKey(record)].filter(Boolean))];
}
...
export function mergeFinancialSignoffs(localRecords = [], backendRecords = []) {
  ...
  backendRecords.forEach((record) => {
    const existing = logicalKeys(record).map((key) => merged.get(key)).find(Boolean);
    if (existing) duplicatesIgnored += 1;
    if (!existing || (['pending_backend', 'sync_error'].includes(existing.syncStatus) && freshness(existing) > freshness(record))) {
      assignRecord(existing || record);
      return;
    }
    assignRecord({ ...existing, ...record, syncStatus: 'synced', syncError: '' });
  });
  return { records: [...new Set(merged.values())], duplicatesIgnored };
}

export function cleanupSyncedFinancialPendingRecords(localRecords = []) {
  const syncedKeys = new Set(localRecords
    .filter((record) => record.syncStatus === 'synced' && (record.backendId || record.localId))
    .flatMap(logicalKeys));
  let removed = 0;
  const records = localRecords.filter((record) => {
    const shouldRemove = ['pending_backend', 'sync_error'].includes(record.syncStatus) && logicalKeys(record).some((key) => syncedKeys.has(key));
  ...
  return {
    records,
    removed,
    localOnlyRemaining: records.filter((record) => ['pending_auth', 'local_only'].includes(record.syncStatus)).length,
  };
}
