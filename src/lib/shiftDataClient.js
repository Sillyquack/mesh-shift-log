...
export function normalizeShiftRecord(row) {
  ...
  return {
    ...
    completedBy: row.operator_name || row.display_name || '',
    role: row.role || '',
    loginSource: row.login_source || '',
    operatorName: row.operator_name || row.display_name || '',
    operatorSource: row.operator_source || row.login_source || '',
    operatorRoleLabel: row.operator_role_label || '',
...
export function normalizeTaskCompletion(row) {
  ...
  return {
    ...
    completedBy: row.operator_name || row.completed_by_name || '',
    operatorName: row.operator_name || row.completed_by_name || inputValues.operatorName || '',
    operatorSource: row.operator_source || inputValues.operatorSource || '',
    operatorRoleLabel: row.operator_role_label || inputValues.operatorRoleLabel || '',
    authDisplayName: row.auth_display_name || inputValues.authDisplayName || '',
...
export function normalizeHandoverNote(row) {
  ...
  return {
    ...
    date: row.note_date || '',
    shiftType: row.shift_key || '',
    completedBy: row.operator_name || row.created_by_name || fields.operatorName || '',
    operatorName: row.operator_name || row.created_by_name || fields.operatorName || '',
    operatorSource: row.operator_source || fields.operatorSource || '',
    operatorRoleLabel: row.operator_role_label || fields.operatorRoleLabel || '',
    authDisplayName: row.auth_display_name || fields.authDisplayName || '',
...
export async function createOrUpdateShiftSession(record) {
  ...
  const payload = {
    ...
    operator_name: record.operatorName || record.displayName || '',
    operator_source: record.operatorSource || record.loginSource || '',
    operator_role_label: record.operatorRoleLabel || '',
    auth_display_name: record.authDisplayName || '',
...
export async function syncTaskCompletion(log, { shiftSessionBackendId = '' } = {}) {
  ...
  const payload = {
    ...
    operator_name: log.operatorName || log.completedBy || '',
    operator_source: log.operatorSource || '',
    operator_role_label: log.operatorRoleLabel || '',
    auth_display_name: log.authDisplayName || '',
    input_values: {
      input: log.input || '',
      comment: log.comment || '',
      inputType: log.inputType || '',
      operatorName: log.operatorName || log.completedBy || '',
      operatorSource: log.operatorSource || '',
      operatorRoleLabel: log.operatorRoleLabel || '',
      authDisplayName: log.authDisplayName || '',
    },
...
export async function syncHandoverNote(note, { shiftSessionBackendId = '' } = {}) {
  ...
  const payload = {
    ...
    note_text: JSON.stringify({
      ...
      operatorName: note.operatorName || note.completedBy || note.createdBy || '',
      operatorSource: note.operatorSource || '',
      operatorRoleLabel: note.operatorRoleLabel || '',
      authDisplayName: note.authDisplayName || '',
    }),
    ...
    operator_name: note.operatorName || note.completedBy || note.createdBy || '',
    operator_source: note.operatorSource || '',
    operator_role_label: note.operatorRoleLabel || '',
    auth_display_name: note.authDisplayName || '',
  };
