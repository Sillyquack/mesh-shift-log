...
import { getRoutineFoundationEditorWorkspace, getRoutineManagerControlCenter, getRoutineTemplateEditorWorkspace } from "./routineManagerClient.js";
...
const stableKey = (entry) => value(entry, "stableKey", "stable_key") ?? value(entry, "standardKey", "standard_key");
...
export async function loadRoutineProductionAmendmentPlan() {
  const [controlCenter, foundation] = await Promise.all([getRoutineManagerControlCenter(), getRoutineFoundationEditorWorkspace()]);
  const { identities, workspaces } = await loadWorkspaces(controlCenter);
  return { plan: buildRoutineProductionAmendmentPlan({ baselinePack, providerPack, foundation, workspaces }), context: { identities } };
}
...
function patchTask(current, operation) {
  ...
  for (const field of operation.fields) {
    ...
    else if (field.field === "doneCriteria") next.doneCriteria = field.after;
    ...
    else if (field.field === "metadata.referenceGuidance") next.metadata = { ...next.metadata, referenceGuidance: field.after };
    else if (field.field === "targetLocalTime") next.targetLocalTime = field.after;
    else if (field.field === "hardDeadlineLocalTime") next.hardDeadlineLocalTime = field.after;
...
function taskMatches(operation, task) {
  ...
  return operation.fields.every((field) => {
    ...
    if (field.field === "doneCriteria") return normalized.doneCriteria === field.after;
    if (field.field === "metadata.timingSourceText") return normalized.metadata.timingSourceText === field.after;
    if (field.field === "metadata.deviationRules") return same(normalized.metadata.deviationRules, field.after);
    if (field.field === "metadata.referenceGuidance") return same(normalized.metadata.referenceGuidance, field.after);
    if (field.field === "targetLocalTime") return normalized.targetLocalTime === field.after;
    if (field.field === "hardDeadlineLocalTime") return normalized.hardDeadlineLocalTime === field.after;
    return false;
  });
}

function normalizeItem(current) {
  return { ...current, itemKey: itemKey(current), itemType: value(current, "itemType", "item_type"), sourceKind: value(current, "sourceKind", "source_kind"),
...
function patchItem(current, operation) {
  const next = normalizeItem(current);
  for (const field of operation.fields) {
    if (field.field === "label") next.label = field.after;
    else if (field.field === "sourceBinding") {
      next.sourceKind = "routine_standard";
      next.standardId = operation.targetStandardId;
      next.sourceLocationSetId = null;
    } else throw new Error(`Unsupported reviewed item field ${field.field}.`);
  }
  next.metadata = { ...next.metadata, sourceText: `\`${next.itemKey}\` — ${next.label}` };
  return next;
}

function itemMatches(operation, item) {
  const current = normalizeItem(item);
  return operation.fields.every((field) => field.field === "label" ? current.label === field.after
    : field.field === "sourceBinding" && current.sourceKind === "routine_standard"
      && current.standardId === operation.targetStandardId && current.sourceLocationSetId === null);
}
...
function standardCurrentValue(standard) {
  const currentRevisionId = value(standard, "currentRevisionId", "current_revision_id");
  return standard.revisions?.find((revision) => revision.id === currentRevisionId)?.value;
}

export async function applyRoutineProductionAmendment({ standardIdempotencyKey }) {
  const initial = await loadRoutineProductionAmendmentPlan();
  const evidence = [];
  for (const operation of initial.plan.operations) {
    try {
      if (operation.kind === "standard_revision") {
        let foundation = await getRoutineFoundationEditorWorkspace();
        let current = foundation.standards.find((entry) => stableKey(entry) === operation.key);
        const beforeRevision = current.revision;
        let mutationError = null;
        try { await routineConfigurationClient.createStandardRevision({ standardId: current.id, value: operation.value, effectiveFrom: null,
          reason: operation.reason, idempotencyKey: standardIdempotencyKey, expectedRevision: beforeRevision }); }
        catch (error) { mutationError = error; }
        foundation = await getRoutineFoundationEditorWorkspace();
        current = foundation.standards.find((entry) => stableKey(entry) === operation.key);
        if (!same(standardCurrentValue(current), operation.value)) throw mutationError || new Error("Serviceware standard readback failed.");
        evidence.push(revisionEvidence(operation, beforeRevision, current.revision, { idempotency: standardIdempotencyKey.slice(0, 8),
          outcome: mutationError ? "authoritative-readback-after-unknown-outcome" : "confirmed" }));
        continue;
      }
      if (operation.kind === "location_set") {
        const foundation = await getRoutineFoundationEditorWorkspace();
        const current = foundation.locationSets.find((entry) => stableKey(entry) === operation.key);
        const beforeRevision = current.revision;
        const description = operation.fields.find((field) => field.field === "description")?.after ?? current.description;
        let mutationError = null;
        try { await routineConfigurationClient.saveLocationSet({ ...current, description, expectedRevision: beforeRevision }); }
        catch (error) { mutationError = error; }
        const readback = (await getRoutineFoundationEditorWorkspace()).locationSets.find((entry) => stableKey(entry) === operation.key);
        if (readback.description !== description) throw mutationError || new Error("Serviceware route scope-anchor readback failed.");
        evidence.push(revisionEvidence(operation, beforeRevision, readback.revision, { idempotency: "optimistic-revision-only",
          outcome: mutationError ? "authoritative-readback-after-unknown-outcome" : "confirmed" }));
        continue;
      }

      const center = await getRoutineManagerControlCenter();
      const identity = templateIdentity(center, operation.routineKey);
      ...
      if (operation.kind === "task") {
        ...
        let mutationError = null;
        try { await routineTemplateClient.saveTask({ versionId: workspace.version.id, sectionId: next.sectionId, taskId: currentTask.id,
          task: buildTaskPayload(next), expectedTaskRevision: beforeTaskRevision, expectedVersionRevision: beforeVersionRevision }); }
        catch (error) { mutationError = error; }
        ...
        evidence.push(revisionEvidence(operation, beforeTaskRevision, readback.revision, { versionBefore: beforeVersionRevision,
          versionAfter: versionRevision(workspace), idempotency: "optimistic-revision-only",
          outcome: mutationError ? "authoritative-readback-after-unknown-outcome" : "confirmed" }));
        continue;
      }
      if (operation.kind === "task_item") {
        ...
        const beforeVersionRevision = versionRevision(workspace);
        const next = patchItem(currentItem, operation);
        let mutationError = null;
        try { await routineTemplateClient.saveItem({ versionId: workspace.version.id, taskId: currentTask.id, itemId: currentItem.id,
          item: { itemKey: next.itemKey, label: next.label, itemType: next.itemType, required: next.required !== false, sourceKind: next.sourceKind,
            ...
            sortOrder: next.sortOrder, active: next.active !== false, metadata: next.metadata }, expectedItemRevision: beforeItemRevision,
          expectedVersionRevision: beforeVersionRevision }); }
        catch (error) { mutationError = error; }
        workspace = await getRoutineTemplateEditorWorkspace(identity.templateId, identity.versionId);
        const readback = workspace.taskItems.find((entry) => value(entry, "taskId", "task_id") === currentTask.id && itemKey(entry) === operation.key);
        if (!itemMatches(operation, readback)) throw mutationError || new Error(`${operation.id} shared-route item readback failed.`);
        evidence.push(revisionEvidence(operation, beforeItemRevision, readback.revision, { versionBefore: beforeVersionRevision,
          versionAfter: versionRevision(workspace), idempotency: "optimistic-revision-only",
          outcome: mutationError ? "authoritative-readback-after-unknown-outcome" : "confirmed" }));
      }
    } catch (error) {
  ...
  if (!final.plan.complete) throw new Error("Reviewed 1.3R serviceware amendment is not complete after authoritative readback.");
