...
import {
  ...
  routineStandardSourceKindLabel,
} from "../data/routineStandardSourceKinds.js";
...
export default function RoutineStandardsManager({ standards, onRefresh, client = routineConfigurationClient }) {
  ...
  return <section className="rm-stack">
    ...
    {selected ? <article className="rm-card">
      <header><div><h4>{selected.label}</h4><code>{selected.stableKey}</code></div><StatusPill state={selected.externalReadonly && selected.stableKey !== SERVICEWARE_ROUTE_STANDARD_KEY ? "warning" : "ready"}>{routineStandardSourceKindLabel(selected.sourceKind)}</StatusPill></header>
      ...
      {selected.externalReadonly && selected.stableKey !== SERVICEWARE_ROUTE_STANDARD_KEY ? <p className="rm-note">{selected.sourceKind === "location_standards" ? "Location-standard values resolve from authoritative inventory location standards and are provider/system managed." : "External source standards are read-only here. Values resolve from the authoritative inventory, asset, or location-set source."}</p> : <div className="rm-form">
