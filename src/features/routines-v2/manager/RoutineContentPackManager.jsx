...
export default function RoutineContentPackManager({
  ...
  const isReviewedAmendment = metadata.packKey === "mesh-routine-content" && metadata.packVersion !== "1.1R";
  ...
  const canInstall = preview.valid && !preview.alreadyInstalled && !isReviewedAmendment && confirmed && note.trim().length >= 8 && status !== "installing";

  return <section className="rm-stack" data-content-pack-state={preview.installStatus}>
    <header className="rm-section-heading"><div><p className="eyebrow">Operational content</p><h2>{metadata.name || "Mesh routine content"}</h2></div><StatusPill state={preview.valid ? (preview.alreadyInstalled || isReviewedAmendment ? "ready" : "warning") : "blocked"}>{preview.alreadyInstalled ? "Installed" : isReviewedAmendment ? "Reviewed amendment" : preview.valid ? "Ready to install" : "Conflict"}</StatusPill></header>
    <section className="rm-hero-card"><div><p className="eyebrow">{metadata.packKey} · v{metadata.packVersion}</p><h3>{isReviewedAmendment ? "Versioned draft amendment" : "Editable content pack"}</h3><p>{isReviewedAmendment ? "This provider can update existing drafts only through the reviewed manager-RPC amendment below; pack installation is unavailable." : "This installs editable drafts only. Nothing is published or operational."}</p><p>Pack SHA-256: <code title={metadata.packHash}>{shortHash(metadata.packHash)}</code></p></div><button type="button" className="ghost-button" disabled={status === "loading" || status === "installing"} onClick={refresh}>{status === "loading" ? "Previewing…" : "Refresh preview"}</button></section>
    ...
    <section className="rm-card"><header><h3>Unresolved publication and readiness requirements</h3><StatusPill state={preview.unresolvedRequirements?.length ? "blocked" : "ready"}>{preview.unresolvedRequirements?.length || 0} blockers</StatusPill></header><ul className="rm-issues rm-blockers">{preview.unresolvedRequirements?.map((entry) => <li key={entry.standardKey}><strong>{entry.label}</strong><br/><small>{entry.standardKey} · affects {entry.affectedTaskIds?.join(", ")}</small></li>)}</ul><p className="rm-note">{preview.unresolvedRequirements?.length ? "These values remain genuinely unresolved. Installation is allowed, but publication and later readiness remain blocked." : "All canonical operational standards in this provider are resolved. Publication and pilot readiness remain separate manager-controlled phases."}</p></section>
