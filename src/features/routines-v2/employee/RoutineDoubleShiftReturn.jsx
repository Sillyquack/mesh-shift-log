...
export default function RoutineDoubleShiftReturn({ context, pending, onReturn }) {
  const [reviewed, setReviewed] = useState(false); return <section className="employee-ds-card"><p className="eyebrow">DS03 · Return</p><h2>Return and review changes before Closing</h2>
    ...
    <button type="button" className="employee-primary" disabled={!context.actions?.canReturnDS03?.allowed || pending || !reviewed} onClick={onReturn}>Join Closing</button></section>;
}
