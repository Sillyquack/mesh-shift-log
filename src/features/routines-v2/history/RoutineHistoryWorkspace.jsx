...
export default function RoutineHistoryWorkspace({ manager = false, loader = listRoutineV2History, runLoader = getRoutineV2HistoryRun, taskLoader = getRoutineV2HistoryTask,
  ...
  return <div className="rh-workspace"><header className="rh-heading"><div><p className="eyebrow">{manager ? "Own-organization authoritative records" : "Only runs you participated in"}</p><h2>{manager ? "History" : "My history"}</h2><p>V2 audit history and legacy records remain clearly separated.</p></div><button type="button" onClick={history.refresh}>Refresh</button></header><nav className="rh-review-sections" aria-label="History workflow"><span className="is-active">Recent</span><span>Find</span><span>Review</span></nav><RoutineHistoryFilters value={filters} onChange={setFilters} manager={manager} />
