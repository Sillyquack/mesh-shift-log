    ...
    <main className="page inventory-workspace counter-workspace">
      ...
      {activeScreen === 'home' ? (
        ...
        <div className="counter-screen" data-counter-screen="count">
          ...
          {!readOnly && <section className="inventory-panel counter-default-panel"><h2>Bruk standard</h2><p className="muted">Physically check this refrigerator first. This fills only eligible, previously uncounted exact-standard lines. Saved quantities, deviations, and comments are preserved; the refrigerator template is never edited.</p><label className="inventory-danger-option"><input type="checkbox" checked={physicalConfirmation} disabled={Boolean(busyId) || summary.unsafeDrafts.length > 0} onChange={(event) => setPhysicalConfirmation(event.target.checked)} /><span>I physically checked this refrigerator</span></label>{summary.unsafeDrafts.length > 0 && <p className="inventory-policy-note">Save or resolve local drafts before applying the standard.</p>}<button type="button" className="secondary-button inventory-full-button" disabled={!physicalConfirmation || Boolean(busyId) || summary.unsafeDrafts.length > 0} onClick={applyDefault}>{busyId === 'default' ? 'Applying…' : 'Apply standard to eligible lines'}</button></section>}
