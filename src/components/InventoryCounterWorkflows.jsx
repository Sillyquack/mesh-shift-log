  ...
  return (
    <article id={`counter-line-${line.id}`} className={`inventory-line-card counter-line-card${deviation ? ' counter-line-deviation' : ''}`} data-line-state={line.countStatus}>
      ...
      <div className="counter-standard-actual" aria-label={`${productLabel} standard and actual quantity`}>
        <div><span>{line.standardQuantityExact === null ? 'Target' : 'Configured target'}</span><strong>{line.standardQuantityExact === null ? 'Physical count only' : `${quantity(line.standardQuantityExact)} ${line.unitLabel}`}</strong></div>
        <div><span>Actual</span><strong>{evaluated.ok ? `${quantity(evaluated.countedQuantity)} ${line.unitLabel}` : 'Not counted'}</strong></div>
      </div>
      {line.historicalSuggestionQuantityExact !== null && <div className="inventory-historical-suggestion"><strong>Previous count suggestion: {quantity(line.historicalSuggestionQuantityExact)} bottle-equivalents</strong><span>{line.historicalSuggestionSource || 'Historical Stock Count'} · this is not a verified current count.</span>{line.historicalSuggestionNote && <p>{line.historicalSuggestionNote}</p>}</div>}
...
function CounterSubmissionReview({ assignment, summary, busy, onBack, onSubmit }) {
  ...
  return (
    <div className="counter-screen" data-counter-screen="review">
      ...
      <section className="inventory-session-header"><div><p className="eyebrow">Review location</p><h2>{assignment.location.name}</h2><p>{assignment.session.title} · {assignment.session.countDate}</p></div><StateBadge state={assignment.state} /></section>
...
export function CounterInventoryWorkspace({ requestWriteAccess, onClose }) {
  ...
  if (loading) return <main className="page inventory-workspace" role="status" aria-live="polite" aria-busy="true"><section className="inventory-panel"><p>Loading your location assignments…</p></section></main>;
  const activeScreen = assignment ? screen : 'home';
  return (
    <main className="page inventory-workspace counter-workspace">
      <header className="inventory-topbar counter-topbar"><button type="button" className="secondary-button" onClick={close}>Log out</button><div><p className="eyebrow">Counter · Mesh Youngstorget</p><h1>Stock Count</h1><p className="muted">Only your assigned locations</p></div></header>
      ...
      {activeScreen === 'home' ? (
        <div className="counter-screen" data-counter-screen="home">
          <section className="inventory-session-header"><div><p className="eyebrow">Counter home</p><h2>Your location assignments</h2><p>Open one location and continue where you left off.</p></div><StatusBadge label={`${assignments.length} assigned`} /></section>
          {!assignments.length ? <section className="inventory-panel inventory-empty"><h2>No active location assignment</h2><p className="muted">Ask Bobby to authorize and assign a location in the active Stock Count.</p><button type="button" className="secondary-button" onClick={refresh}>Refresh assignments</button></section> : <div className="inventory-line-list counter-assignment-list">{assignments.map((item) => <CounterAssignmentCard key={item.id} assignment={item} drafts={drafts} onOpen={openAssignment} />)}</div>}
        </div>
      ) : activeScreen === 'review' ? (
        ...
        <div className="counter-screen" data-counter-screen="count">
          ...
          <LocationReferenceViewer locationName={assignment.location.name} guidance={assignment.referenceGuidance || { locationId: assignment.location.id }} />
          ...
          {assignment.state === 'submitted' && <section className="inventory-panel counter-readonly-state"><strong>Sent to Bobby — waiting for review</strong><p className="muted">This location is submitted and read-only. Bobby may accept it or return it with a message.</p></section>}
          {assignment.state === 'accepted' && <section className="inventory-panel counter-readonly-state"><strong>Accepted by Bobby</strong><p className="muted">This location is read-only and finished. Session completion and approval remain manager actions.</p></section>}
          {assignment.state === 'superseded' && <section className="inventory-panel counter-readonly-state"><strong>Assignment replaced</strong><p className="muted">This location is no longer actionable from this counter account.</p></section>}
          {!readOnly && <section className="inventory-panel counter-default-panel"><h2>Bruk standard</h2><p className="muted">Physically check this location first. This fills only eligible, previously uncounted exact-target lines. Saved quantities, targetless lines, historical suggestions, deviations, and comments are preserved; configuration is never edited.</p><label className="inventory-danger-option"><input type="checkbox" checked={physicalConfirmation} disabled={Boolean(busyId) || summary.unsafeDrafts.length > 0} onChange={(event) => setPhysicalConfirmation(event.target.checked)} /><span>I physically checked this location</span></label>{summary.unsafeDrafts.length > 0 && <p className="inventory-policy-note">Save or resolve local drafts before applying the standard.</p>}<button type="button" className="secondary-button inventory-full-button" disabled={!physicalConfirmation || Boolean(busyId) || summary.unsafeDrafts.length > 0} onClick={applyDefault}>{busyId === 'default' ? 'Applying…' : 'Apply exact targets to eligible lines'}</button></section>}
          ...
          {!readOnly && <section className="inventory-panel counter-extra-note"><h2>Annen vare eller avvik</h2><p className="muted">There is no counter-safe full-catalogue extra-product lookup. Record an observed extra or relevant comment in the closest product’s comment field above; it stays inside this assigned location.</p></section>}
          {!readOnly && <section className="inventory-panel counter-submit-panel"><h2>Review before sending</h2><p className="muted">Check incomplete products, deviations, comments, and save status before sending this location to Bobby.</p><button type="button" className="primary-button inventory-full-button" disabled={Boolean(busyId)} onClick={() => setScreen('review')}>Review location</button></section>}
        </div>
      )}
    </main>
  );
...
export function CounterAssignmentManager({ data, activeSession, lines, requestWriteAccess, refresh, setStatus }) {
  ...
  const assignableLocations = data.locations.filter((item) => item.active !== false && item.countable === true && sessionLocationIds.has(item.id) && !assignedLocationIds.has(item.id));
  ...
  return (
    <div className="inventory-stack">
      ...
      <section className="inventory-panel">
        <div className="inventory-panel-heading"><div><p className="eyebrow">Active Stock Count</p><h2>Assign countable locations</h2></div></div>
        {!activeSession ? <p>No active Stock Count is available.</p> : <><p>{activeSession.title} · {activeSession.countDate}</p><div className="inventory-form-grid"><label>Counter<select value={counterMembershipId} onChange={(event) => setCounterMembershipId(event.target.value)}><option value="">Choose counter</option>{activeMemberships.map((membership) => <option key={membership.id} value={membership.id}>{profilesById.get(membership.counterAuthUserId)?.displayName || 'Counter'}</option>)}</select></label><label>Countable location<select value={locationId} onChange={(event) => setLocationId(event.target.value)}><option value="">Choose location</option>{assignableLocations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label></div><button type="button" className="primary-button" disabled={!counterMembershipId || !locationId || Boolean(busyId)} onClick={() => run('assign', () => createInventoryCountAssignment({ sessionId: activeSession.id, locationId, counterMembershipId, expectedSessionUpdatedAt: activeSession.updatedAt }))}>{busyId === 'assign' ? 'Assigning...' : 'Assign location'}</button></>}
      </section>
