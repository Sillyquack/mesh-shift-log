...
function StateBadge({ state }) {
  const label = ({ assigned: 'In progress', submitted: 'Sent to Bobby', returned: 'Returned', accepted: 'Accepted' })[state] || state;
  const tone = state === 'accepted' ? 'good' : state === 'returned' ? 'warning' : state === 'submitted' ? 'active' : '';
...
export function CounterAssignmentManager({ data, activeSession, lines, requestWriteAccess, refresh, setStatus }) {
  ...
  const assignedLocationIds = new Set(assignments.map((item) => item.locationId));
