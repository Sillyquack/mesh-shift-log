...
function CountLineCard({ line, identityReference, draftValue, setDraftValue, caseDraft, setCaseDraft, note, setNote, action, busy, readOnly, canManage }) {
...
function CountSession({ session, sessions, lines, locations, canManage, canCoordinate, requestWriteAccess, onRefresh, onOpenSession, onBack, setStatus, remoteNotice, clearRemoteNotice }) {
  ...
  const exportSession = () => downloadCsv(`mesh-stock-count-${session.countDate}.csv`, makeCsv(['Date', 'Session', 'Status', 'Location', 'Product', 'Product ID', 'Unit', 'Stock policy', 'Target', 'Counted', 'Gap', 'Count method', 'Note', 'Counted by', 'Counted at'], orderedLines.map((line) => { const calculated = calculateInventoryLine(line); return [session.countDate, session.title, session.status, line.locationName, line.productName, line.productId, line.unitLabel, line.stockPolicy, calculated.effectiveTarget, line.countedQuantity, calculated.restockQuantity, line.countMethod, line.note, line.countedByName, line.countedAt]; })));
  return (
    <div className="inventory-stack">
...
function RestockView({ session, lines }) {
  ...
  const exportRestock = () => downloadCsv(`mesh-restock-${session?.countDate || osloDate()}.csv`, makeCsv(['Product', 'Product ID', 'Unit', 'Location', 'Missing quantity', 'Category'], restock.flatMap((product) => product.locations.map((location) => [product.productName, product.productId, product.unitLabel, location.locationName, location.missingQuantity, product.category]))));
  return <div className="inventory-stack"><section className="inventory-panel"><div className="inventory-panel-heading"><div><p className="eyebrow">Daily and operating stock</p><h2>Restock list</h2></div><button type="button" className="secondary-button" disabled={!restock.length} onClick={exportRestock}>Export CSV</button></div><p className="muted">Exact-par shortages and operating-reserve gaps only. This is not a transfer order and does not claim storage stock is available.</p>{restock.length ? restock.map((product) => { const reference = inventoryProductIdentityReference(product, restock); return <article className="inventory-restock-card" key={product.productId}><h3>{product.productName}</h3>{reference && <p className="inventory-audit">{reference}</p>}<p><strong>Total gap: {quantity(product.totalMissing)} {product.unitLabel}</strong></p>{product.locations.map((location) => <p key={location.lineId}>{location.locationName}: {quantity(location.missingQuantity)}</p>)}</article>; }) : <p>No daily or operating-reserve requirements in the selected session.</p>}</section>{eventReserve.length > 0 && <section className="inventory-panel"><div><p className="eyebrow">Protected stock</p><h2>Event reserve readiness</h2></div><p className="inventory-protected-note"><strong>Not for daily restocking</strong><span>Event reserve remains separate from Main beverage stock. No transfer is created here.</span></p>{eventReserve.map((item) => { const reference = inventoryProductIdentityReference(item, eventReserve); return <article className="inventory-restock-card" key={item.id}><h3>{item.productName}</h3>{reference && <p className="inventory-audit">{reference}</p>}<p><strong>{item.readinessPercent ?? 0}% ready</strong></p><p>{item.countFullCases ?? '-'} / {item.targetCases ?? '-'} cases · {quantity(item.actualUnits)} / {quantity(item.targetUnits)} units</p><p>{quantity(item.shortageUnits)} units short</p></article>; })}</section>}</div>;
}
...
function ProductManager({ products, run }) {
  ...
  const exportCatalog = () => downloadCsv('mesh-inventory-products.csv', makeCsv(['Product name', 'Product ID', 'SKU', 'Barcode', 'Category', 'Unit', 'Active', 'Supplier'], products.map((item) => [item.name, item.id, item.sku, item.barcode, item.category, item.unitLabel, item.active ? 'yes' : 'no', item.supplierName])));
  return (
    <section className="inventory-panel">
      ...
      <div className="inventory-config-list">{visibleProducts.map((item) => { const reference = inventoryProductIdentityReference({ ...item, productId: item.id, productName: item.name }, identityPeers); return <article key={item.id}><div><strong>{item.name}</strong><span>{item.category || 'Other'} · {item.unitLabel}{reference ? ` · ${reference}` : ''}{!item.active ? ' · Archived' : ''}</span></div><div><button type="button" className="secondary-button" onClick={() => setProduct(item)}>Edit</button><button type="button" className="text-button" onClick={() => run(() => saveInventoryProduct({ ...item, active: !item.active }))}>{item.active ? 'Archive' : 'Reactivate'}</button></div></article>; })}</div>
    </section>
  );
}
