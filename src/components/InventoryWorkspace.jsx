...
import { InventoryLocationAlignmentManager } from './InventoryLocationAlignment.jsx';
import { locationCountabilityLocked } from '../data/inventoryLocationAlignment.js';
...
export function approvedMonthlySessionForDate(sessions, date = osloDate()) {
  const month = String(date || '').slice(0, 7);
  if (!month) return null;
  return sessions.find((session) => session.status === 'approved'
    && session.countType === 'monthly'
    && (session.sessionKind || 'standard') === 'standard'
    && String(session.countDate || '').startsWith(`${month}-`)) || null;
}
...
function InventoryOverview({ sessions, activeSession, lines, locations, onOpenSession, onStart, canCoordinate }) {
  ...
  const approvedThisMonth = approvedMonthlySessionForDate(sessions);
  return (
    <div className="inventory-stack">
      {activeSession ? (
        ...
        <section className="inventory-panel inventory-empty inventory-month-complete">
          <p className="eyebrow">Monthly count complete</p>
          <h2>This month is already approved</h2>
          <p className="muted">{approvedThisMonth.title} was approved {formatDateTime(approvedThisMonth.approvedAt)}. A correction should be started from the approved count, so a second monthly count is not offered here.</p>
          <button type="button" className="primary-button" onClick={() => onOpenSession(approvedThisMonth.id)}>View approved count</button>
        </section>
      ) : (
...
function CatalogManager({ data, requestWriteAccess, refresh, setStatus }) {
  ...
  return <div className="inventory-stack"><nav className="inventory-subtabs"><button type="button" className={view === 'layout' ? 'active' : ''} onClick={() => setView('layout')}>Physical layout</button><button type="button" className={view === 'fridges' ? 'active' : ''} onClick={() => setView('fridges')}>Fridge defaults</button><button type="button" className={view === 'products' ? 'active' : ''} onClick={() => setView('products')}>Products</button><button type="button" className={view === 'locations' ? 'active' : ''} onClick={() => setView('locations')}>Locations</button><button type="button" className={view === 'standards' ? 'active' : ''} onClick={() => setView('standards')}>Standards</button><button type="button" className={view === 'guidance' ? 'active' : ''} onClick={() => setView('guidance')}>Reference images</button><button type="button" className={view === 'import' ? 'active' : ''} onClick={() => setView('import')}>CSV import</button></nav>{view === 'layout' && <InventoryLocationAlignmentManager data={data} onOpenStandards={() => setView('standards')} onOpenGuidance={() => setView('guidance')} />}{view === 'fridges' && <RefrigeratorDefaultsManager products={products} locations={locations} standards={standards} refrigeratorTemplates={refrigeratorTemplates} unresolvedMappings={unresolvedMappings} reserves={reserves} requestWriteAccess={requestWriteAccess} refresh={refresh} setStatus={setStatus} />}{view === 'products' && <ProductManager products={products} run={run} />}{view === 'locations' && <LocationManager locations={locations} requestWriteAccess={requestWriteAccess} refresh={refresh} run={run} setStatus={setStatus} />}{view === 'standards' && <StandardsManager products={products} locations={locations} standards={standards} storageSettings={storageSettings} requestWriteAccess={requestWriteAccess} refresh={refresh} run={run} setStatus={setStatus} />}{view === 'guidance' && <LocationReferenceGuidanceManager data={data} requestWriteAccess={requestWriteAccess} refresh={refresh} setStatus={setStatus} />}{view === 'import' && <CsvImport products={products} locations={locations} requestWriteAccess={requestWriteAccess} refresh={refresh} setStatus={setStatus} />}</div>;
}
...
function AuthorizedInventoryWorkspace({ user, requestWriteAccess, onClose }) {
  ...
  const approvedThisMonth = approvedMonthlySessionForDate(data.sessions);
  ...
  return (
    <main className="page inventory-workspace">
      <header className="inventory-topbar"><div><p className="eyebrow">Mesh Youngstorget</p><h1>Stock Count</h1></div><Status tone={realtimeStatus.state === 'connected' ? 'good' : ''}>{realtimeStatus.state === 'connected' ? 'Live' : 'Polling backup'}</Status></header>
