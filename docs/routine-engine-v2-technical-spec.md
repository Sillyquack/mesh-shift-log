...
`npm run verify:routine-full-migration-reapply` uses a disposable PostgreSQL 17.6 container with Docker network mode `none`, a production-shaped synthetic baseline and protected-domain fingerprints. It applies Phase 10A–10L three times, audits raw `pg_proc.proacl` plus effective privileges through `aclexplode(coalesce(proacl, acldefault('f', proowner)))`, and requires identical Routine schema, raw ACL, effective ACL, data, revision, timestamp and operational fingerprints after every sequence. It also exercises canonical and rejected validator argument names, anon denial, public authenticated reads/mutations, private-helper denial, staff/shared-device manager denial and internal security-definer call chains. The verified fingerprints are Routine schema `7665b5ebc3eb24f239888188601c4e5b4a1e47b77fbad69ddef23cc58af759d4`, raw ACL `ac86a7505043d1db53b1b9bd8d9b43561b520a7bf1c8d5a99e8d09f55e17a2ad`, and effective ACL `2ef51b540d59b3288f508993ce0d6b6f824fa2e4300cf219f11f23e3956fa9c4`. This verification performs no production migration or data operation.

## Phase 10L versioned Mesh content pack

Phase 10L starts from the clean K4 checkpoint `fc8225d66fe702f0959765f499cc6cf7aebe0166` (`feat: add routine history and pilot hardening`). The single canonical manifest is `content/routine-engine/mesh-routine-content-v1.json`, generated from the three re-locked Opening, Closing and Double Shift sources after exact SHA-256 verification. `mesh-routine-content@1.0R` has pack hash `d8daf5e8c887c59023a99b741bc5f13ba46b4e74f23b4b003583eafc9f17c574`, 37 Opening tasks in three sections, 46 Closing tasks in two sections, and four Double Shift system-step definitions plus bundle copy. Double Shift remains a bundle over linked Opening and Closing runs and is never a third routine template.
...
The pack defines 43 locations without room 005, 12 ordered sets, 14 standards and 33 logical reference placeholders. Five standards have locked values, including Workbar Milk Fridge at 2 regular milk plus 2 Oatly, four lunch Coffee Canisters and daily baked goods. Nine standards deliberately have no current revision: full and service-ready cup/glass targets, total Coffee Canister inventory, six tea-slot names, office-floor serviceware recovery, door/lock rules and fridge-closing rules. These remain publication and readiness blockers; no zero, empty string, `TBD` or guessed location/rule satisfies validation. Reference placeholders contain no physical image and retain the Phase 10C prepare → server path → upload → finalize workflow for later manager uploads.

The source-derived task model retains every instruction, structured item, done criterion, deviation/blocking rule and reference-guidance block with a source-block hash. It preserves O13 eggs, independent O29/O35 physical final-restock checks, O27→O29 and O33→O35 continuous completion, Previous Closing evidence as context only, and transfer/evidence handling for event-active work. Cross-run relations are declarative and never auto-complete Opening; delivery comparisons use same-scope typed evidence. The generated audit document lists every O/C task’s section, type, criticality, mandatory state, policies, timing, location/set, item count and source hash.

`get_mesh_routine_content_pack_audit()` returns installation and canonical hashes, installed draft IDs/hashes, exact/missing/duplicate O/C IDs, titles, sections, item/reference/dependency/relation counts, DS definitions, unresolved requirements, operation history and semantic divergence. A later manager edit is reported as changed from the installed pack and is not auto-repaired. Release readiness models operational content as two published templates plus four DS system steps; installation remains a draft-only state and all unresolved standards keep readiness blocked.

Phase 10L adds the lazy isolated `RoutineContentPackManager` under `routines-v2/manager/`; `App.jsx` remains unchanged. Frontend writes use only the content installer or the existing Phase 10A–10J mutation RPCs. The preview, note and stable idempotency key survive stale, network, auth and validation failures, and no automatic rebase occurs. The generated SQL, human audit document and canonical manifest are compared deterministically, including a fresh rebuild from all three locked sources.

No Supabase production migration, content installation, publication, run/task/bundle/delivery creation, mode/stage change, pilot activation, Auth change, Stock Count/Inventory Storage change, Event Operations change, legacy write, deployment, push or PR has been performed for Phase 10A through 10L. K4 is the committed checkpoint above; Phase 10L remains an uncommitted working-tree implementation. The accepted Phase 9L baseline remains exactly `Phase 9L requires exactly one approved August shelf/storage source session.`
