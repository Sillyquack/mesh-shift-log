...
`npm run verify:routine-full-migration-reapply` uses a disposable PostgreSQL 17.6 container with Docker network mode `none`, a production-shaped multi-organization baseline and protected-domain fingerprints. It applies the 16 migrations from Phase 10A through 10L three times—48/48 applications—with no out-of-band manager bootstrap. The first sequence asserts zero settings rows after 10A, exact missing-row bootstrap after 10A1, `foundation/phase10k1-v1/revision 1` after K1, `manager_preview/phase10k2-v1/revision 2` after K2, `staff_preview/phase10k3-v1/revision 3` after K3, and `staff_preview/phase10k4-v1` with inert pause defaults after K4; 10L leaves the same state and installs nothing. A non-default pre-existing settings row is preserved by 10A1, and later reapply sequences are data/revision/timestamp stable. The verifier audits raw `pg_proc.proacl` plus effective privileges through `aclexplode(coalesce(proacl, acldefault('f', proowner)))` and requires identical Routine schema, raw ACL, effective ACL, data and operational fingerprints after every sequence. With the 1.1R embedded payload, the verified fingerprints remain Routine schema `f44dc34640c684d2dc1cbb072f88f7d111c49f13a9b5a934905a07c73733c33a`, raw ACL `ac86a7505043d1db53b1b9bd8d9b43561b520a7bf1c8d5a99e8d09f55e17a2ad`, and effective ACL `2ef51b540d59b3288f508993ce0d6b6f824fa2e4300cf219f11f23e3956fa9c4`. This verification performs no production migration or data operation.
