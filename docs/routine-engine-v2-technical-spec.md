...
Phase 10A1 is the reviewable organization-settings bootstrap added after the production preflight stopped before 10A. The stop exposed that 10A creates `routine_organization_settings` and its manager RPC but intentionally creates no row for organizations that already exist. No production migration had started. The uncommitted 10A1 migration inserts only missing rows with inert defaults, null system actor audit, deterministic organization ordering and conflict no-op behavior; it never calls or impersonates a manager and never changes an existing settings row.

The previously approved recovery snapshots remain untouched and authoritative. This local implementation grants no production authorization: after 10A1 is reviewed and merged, Gate 0, Gate 1, the fresh backup condition and the final zero-delta write-quiescence gate must all be rerun against the new exact release commit before a separately explicit production authorization can permit 10A.
...
The additive chain begins with `supabase/phase10a_routine_engine_foundation.sql`, immediately follows with `supabase/phase10a1_routine_organization_settings_bootstrap.sql`, and then continues through the remaining Phase 10 migrations. Every Routine Engine v2 table is in `public`, has RLS enabled, and has a non-null organization boundary.
...
## Phase 10K4 History and pilot hardening

Phase 10K4 advances only existing K3 contract rows to `phase10k4-v1`. It leaves `staff_preview`, mode, pause state and all operational content unchanged and is revision/timestamp stable on reapply. Two no-direct-DML tables hold immutable release and disposable E2E verification attestations. Personal-manager RPCs compute a deterministic fifteen-category readiness snapshot and SHA-256 hash, record E2E evidence, attest/promote only `staff_preview → pilot_ready`, and control pilot new-work pause. `production_ready` and `active` remain unavailable.

The K4 wrapper around the existing mode RPC preserves legacy/shadow behavior. `shadow → pilot` additionally requires current green readiness, a matching immutable accepted attestation and an active participant membership. `pilot → shadow` refuses while a run or Double Shift bundle is active. The new-work pause wraps only run creation, bundle creation and scheduled-run start; already active work, participation, lifecycle, deviation, handover, transfer and finish RPCs remain available. K4 installs these paths but calls none of them outside disposable tests.

Six bounded, tenant-strict v2 history/review RPCs expose authoritative run/task evidence, participants and effective identity source, events, timing, deviations, overrides, verifications, transfers, deliveries, comparisons, Double Shift, sync evidence and separate corrections. Staff/shared operators receive only runs in which their effective identity participated; broad review, correction lists, override follow-up and legacy access require personal manager auth. The UI operation ledger is deliberately omitted from ordinary timelines.

The legacy adapter reads only explicitly same-organization rows from `shift_sessions`, `task_completions`, `handover_notes`, `close_day_archives` and `manager_daily_reviews`. Each record says `legacy_shift_log`, `legacy_record_only`, and lists available/unavailable fields. NULL-organization rows are never returned, inferred or assigned; only their aggregate count is available to a personal manager and appears as a readiness warning. The adapter creates no trigger or legacy policy and performs no legacy DML.

History, Operational Review and Release Gate are isolated below `routines-v2/history/` and loaded as dedicated production chunks from the existing manager/employee workspaces. Manager actions delegate to existing Phase 10E mutation RPCs, preserve local forms on stale/auth/network/validation failure, and keep corrections/overrides visually separate from successful outcomes. Chunk-load failure offers a controlled reload without sending a mutation. The service worker remains network-first for navigation; its cache version is bumped because the previous catch-all same-origin GET cache could retain reverse-proxied Auth/API/Storage responses. K4 caches only the shell and versioned `/assets/`, while credential-bearing requests and `/auth`, `/rest`, `/storage`, and `/functions` stay network-only. The production rollout runbook documents preflight, backup, exact migration order, Storage/Realtime/RLS checks, shadow-first rollout, 10L content, attestations, pilot activation, emergency pause and immutable-history-safe rollback.

The final local K4 release-candidate run passed all 36 declared visual scenarios and all 226 E2E/browser checks: 26 scenarios in Chromium and 10 in WebKit, including 320 px and iPhone-like 390×844 layouts, dark mode, 200% zoom, reduced motion, offline/reconnect, IndexedDB, ephemeral sessionStorage operator-token boundaries, lazy-chunk recovery and two simultaneous contexts in each engine. A loopback-locked test-only Vite proxy connected the actual production Routine Engine clients and components to PostgREST over a disposable PostgreSQL 17 database for positively scoped personal-staff and shared-operator history plus manager pilot/rollback evidence; manager scenarios also replayed the real mode and pause mutation clients through server idempotency without changing state. The database remained on its internal Docker network and the production bundle contained neither proxy configuration nor harness code. Every scenario had zero unexpected console errors, no Vite overlay, no horizontal page overflow, zero automated accessibility violations and compliant 48-pixel minimum touch targets where controls were present. Keyboard-only navigation, logical focus, dialog focus trap/Escape/return and screen-reader-friendly status/timeline behavior were also exercised. Screenshots were written only to a disposable directory outside the repository. The exact accepted Phase 9L result remains `Phase 9L requires exactly one approved August shelf/storage source session.`
...
4. `supabase/phase10a1_routine_organization_settings_bootstrap.sql`;
5. `supabase/phase10b_routine_templates.sql`;
6. `supabase/phase10c_routine_reference_images.sql`;
7. `supabase/phase10d_routine_runs_and_snapshots.sql`;
8. `supabase/phase10e_routine_task_lifecycle.sql`;
9. `supabase/phase10f_routine_operational_time.sql`;
10. `supabase/phase10g_routine_closing_delivery.sql`;
11. `supabase/phase10h_routine_double_shift.sql`;
12. `supabase/phase10i_routine_realtime_offline_sync.sql`;
13. `supabase/phase10j_routine_shared_device_identity.sql`;
14. `supabase/phase10k1_routine_ui_pilot_gate.sql`;
15. `supabase/phase10k2_routine_manager_control_center.sql`;
16. `supabase/phase10k3_routine_employee_workflow.sql`;
17. `supabase/phase10k4_routine_history_pilot_hardening.sql`;
18. `supabase/phase10l_mesh_routine_content_pack.sql`;
19. `supabase/phase10p_routine_readiness_finalization.sql`;
20. `supabase/phase10q_mesh_routine_content_pack_1_2r.sql`;
21. `supabase/phase10o_routine_default_privilege_hardening.sql`;
22. `supabase/phase10r_mesh_routine_content_pack_1_3r.sql`;
23. `supabase/phase10s_mesh_routine_content_pack_1_4r.sql`;
24. `supabase/phase10t_routine_participant_identity_conflict_alignment.sql`;
25. `supabase/phase10u_routine_operation_idempotency_convergence.sql`;
26. `supabase/phase10v_routine_creation_idempotency_provenance_alignment.sql`;
27. `supabase/phase10w_event_visual_reference_bridge.sql`;
28. `supabase/phase10x_event_visual_library_expansion.sql`;
29. `supabase/phase10y_mesh_routine_content_pack_1_5r.sql`;
30. `supabase/phase10z_inventory_location_and_express_shelf_alignment.sql`;
31. `supabase/phase10aa_event_floor_manager_pilot_membership.sql`.

All 29 Phase 10 migrations through 10AA are safe to reapply against their own completed schema. Phase 10A1 is insert-only for missing organizations, so existing rows, revisions and timestamps are stable. Phase 10I preserves instance/receipt/reconciliation rows and timestamps, adds `routine_events` publication membership only once, and recreates functions, policies, and triggers without repairing or reordering earlier migrations. Phase 10J preserves all existing hashes and historical rows, and its own reapplication preserves device/operator/session/audit rows and timestamps. Phase 10K1 preserves pilot/settings/operation rows and timestamps. Phase 10K2 advances only existing foundation-stage rows, preserves later stages, settings timestamps and every earlier hash/protected-domain fingerprint, and writes no run, task or template row. Phase 10K3 advances only existing manager-preview rows, preserves later stages and all operational data/timestamps, and grants six read-only employee projections without adding a write contract. Phase 10K4 advances only the contract version of existing K3 rows, leaves release stage and mode unchanged, adds immutable attestations and manager/participant history projections, and installs—but never invokes—the controlled pilot gates. Phase 10L installs only the schema, immutable canonical read function, manager preview/install/audit RPCs, readiness override and exact grants; applying the migration alone creates no organization content. Phase 10P replaces only the private pilot-readiness computation so existing same-organization current standard revisions are recognized and stale pointers remain blocking. Phase 10Q, 10R, 10S and 10Y replace only the private canonical pack provider with their reviewed content revisions; none installs content or alters drafts. Phase 10O changes only the effective migration role's defaults for future public tables, sequences and functions; existing object grants and rows remain byte-/record-identical. Phase 10T aligns five participant conflict targets with the final partial identity indexes. Phase 10U replaces only the four operation-ledger replay/writer functions, adds transaction-scoped identity locks plus conflict-safe immutable readback, and leaves schema, grants, content and operative rows unchanged. Phase 10V removes four obsolete organization-global resource/participant creation-key constraints; operation ledgers remain the replay authority, while the stored resource key remains immutable creation provenance and business/participant partial indexes remain authoritative. Phase 10W adds the least-privilege Event visual bridge, Phase 10X expands its exact allowlist and closes anonymous Event Ops execution, Phase 10Y exposes only the additive 1.5R refrigerator-standards provider, Phase 10Z performs the guarded inventory-location alignment, and Phase 10AA replaces only the pilot desired-state RPC so personal Event Floor Managers may receive preview or participant—but never coordinator—access.

## Defaults and open configuration

Phase 10A defines conservative defaults but does not create an organization settings row. Phase 10A1 creates the missing row for every existing organization with those exact defaults and null system actor audit:
...
- UI contract: `phase10k1-v1` at K1 foundation, `phase10k2-v1` after manager preview, `phase10k3-v1` after the staff-preview transition, and `phase10k4-v1` after history/pilot hardening;
- reopen window: 24 hours, constrained to 0–168 hours.

Before a later rollout phase, product owners must approve the organization rollout mode, location catalog and hierarchy, location-set membership, the complete `O`, `C`, and `DS` content mappings, standard values/units/effective dates, source-adapter behavior, approved actual Mesh reference images, and whether shared-device operation is introduced. No value in this list is activated by Phase 10A, 10A1, 10B, 10C, or 10D. No `O01`–`O37`, `C01`–`C46`, `DS01`–`DS04`, random illustration, actual Mesh image, production run, or production snapshot is seeded; content is deferred to Phase 10L.

## Verification and known baseline

In the verifier summaries below, every range beginning at Phase 10A expands to the ordered prefix `10A → 10A1 → 10B`; no harness may skip the bootstrap migration.

`npm run verify:routine-foundation` starts a uniquely named, network-isolated disposable Supabase PostgreSQL container. Foundation now means 10A + 10A1: the verifier proves 10A creates an empty settings table, installs multi-organization fixtures plus one non-default pre-existing row, applies 10A1, validates exact defaults/null actor audit/cross-tenant separation, and proves immediate reapply preserves data, revisions and timestamps. It also verifies authenticated settings INSERT/UPDATE/DELETE and anon reads remain denied, the later manager RPC still works, protected Inventory/legacy fingerprints remain unchanged, and concurrent standard revisions serialize correctly.

`npm run verify:routine-templates` applies Phase 10A, 10A1 and 10B twice in another uniquely named, network-isolated disposable PostgreSQL 17 container. It executes 94 SQL assertions across schema, immutability, lifecycle, validation, publishing, RLS, and regression boundaries. Two real connections prove that concurrent draft creation produces at most one draft and that concurrent identical publish calls converge on one immutable publication batch. It fingerprints Inventory, legacy routine, Event Operations, and Auth objects and verifies data-stable migration reapplication.

`npm run verify:routine-reference-images` applies Phase 10A, 10A1, 10B, and 10C to another uniquely named, network-isolated disposable PostgreSQL 17 container. It executes 151 SQL assertions across schema, lifecycle, immutable versions/links, hashing, validation, publishing, Storage policy behavior, cleanup, paths, RLS, grants, and protected-domain regressions. Two real connections prove unique concurrent version allocation with stale rejection and convergence of identical finalize calls on one immutable operation. The runner also tests JPEG/PNG/WebP magic bytes, MIME mismatch, oversize rejection, and deterministic client normalization without network access. It fingerprints Inventory Storage and protected Inventory, legacy, Event Operations, and Auth schemas, then proves Phase 10B-plus-10C reapplication is data- and timestamp-stable.
...
## Full-chain migration reapply and function ACL contract

The public validator contract is `validate_routine_template_version(input_version_id uuid, input_publication_version_ids uuid[])`. Every Phase 10 definition preserves those input names, order and types because PostgreSQL rejects `CREATE OR REPLACE FUNCTION` when an existing identity signature is given different input parameter names. The layered Phase 10C–10H validation logic remains unchanged; no alias function or drop/recreate workaround is used.

PostgreSQL grants new functions `EXECUTE` to `PUBLIC` by default, and the Supabase-shaped baseline also supplies established platform-role defaults. Earlier phases therefore reassert exact existing-object ACLs signature by signature. Phase 10O adds the distinct future-object boundary: in one explicit transaction it revokes client defaults for public-schema tables, sequences and functions from the effective `current_user`, without `FOR ROLE`. PostgreSQL's built-in global `PUBLIC EXECUTE` function default cannot be removed by a schema-scoped revoke, so 10O also performs the required global future-function revoke. There is no all-existing-functions revoke, existing-object GRANT/REVOKE, public RPC, DML, owner change, policy change, `DROP FUNCTION`, or `CASCADE` operation.

`npm run verify:routine-full-migration-reapply` uses network-isolated disposable PostgreSQL 17.6 databases in rehearsal (`session_user=phase10o_rehearsal_login`, `current_user=supabase_admin` through `SET ROLE`) and production-shaped (`session_user=current_user=postgres`) owner contexts. The temporary rehearsal login exists only inside the disposable database and is explicitly made a member of the migration owner. Each context applies all 29 migrations through Phase 10AA three times—87/87 applications—with no out-of-band manager bootstrap. Before and after 10O it compares every existing Routine relation/function/schema ACL, policy, function body/config, relation definition, Storage policy, Realtime membership, settings row, Routine row count and protected-domain fingerprint; only `pg_default_acl` may change. The 10V step additionally proves that exactly four legacy creation-key constraints and their owned indexes are absent while all six resource/participant business-identity indexes remain valid. Phase 10AA runs a rollback-isolated database matrix proving Event Floor Manager preview/participant acceptance, coordinator denial, the unchanged staff/Shift Lead rules, every excluded profile class, exact replay, atomic replacement, revision safety and immutable auditing. Future table/sequence/function probes must be owned by the same effective role as `pg_default_acl.defaclrole`, expose no client privilege and retain owner access. Negative probes reintroduce each default privilege and must be detected.

The portable semantic schema contract is `routine-canonical-catalog-forensics-v1`. It deparses with `search_path=pg_catalog`, uses schema-qualified identities and deterministic category/identity ordering, replaces only object-owner values with `<OBJECT_OWNER>`, and excludes relation/function/schema/default ACL records. It includes schemas, relation and column semantics, constraints/FK targets, indexes/predicates, types/enums/domains, triggers, full function signature/config/body hashes, RLS/policies, the Storage policy/bucket contract and Routine Realtime membership. Through the terminal Phase 10AA function-only correction, its accepted fingerprint is `f53315ccdfa8d2636c0baad8cd8c3a9d90e5a4033a9225cb0b7493c6ffb05f4f` in both owner contexts. Real disposable column/default, FK, index, policy, function-body, search-path and forced-RLS mutations must all fail this gate.

Client ACL and default ACL are separate blocking attestations, never owner-normalized. Existing access requires zero `PUBLIC`/`anon` Routine function execution; exactly 218 authenticated function signatures (hash `61446c15b10333748c65a652f01f6c9e91df67b81593f4db65bc7f0c2bee2a0e`); 32/32 reviewed public/internal signatures; zero direct client Routine DML; exactly 65 authenticated SELECT relations (hash `9f02d0f0f22ef6c607f793210fda83deb88e23cbb68943834ba41dabb206f5bd`); zero unconditional or broad (`PUBLIC`/`anon`) Routine RLS; and exact validator names. PostgreSQL's policy-combination label `PERMISSIVE` is not itself a broad predicate; the verifier rejects public/anon policy roles and literal true predicates. Effective future-object defaults, including implicit defaults, must give `PUBLIC`, `anon` and `authenticated` no table, sequence or function privilege. Actual owners, migration role, `postgres`, `supabase_admin`, `service_role`, privileged grants and default-ACL owner context are a non-portable environment report. Routine ownership accepts only `postgres`/`supabase_admin`; the canonical public/Storage infrastructure may additionally report only `pg_database_owner`/`supabase_storage_admin`. The previous schema/raw/effective ACL hashes remain printed only as environment-dependent diagnostics.

The reviewed production baseline is `shadow/staff_preview` with Phase 10O already applied, two unpublished editable drafts, and zero Routine operative rows. Phase 10P/10Q source presence is not production authorization: each forward application still requires the production gate and post-application attestations. No database rollback is required. This verification performs no production migration or data operation.

## Phase 10L versioned Mesh content pack

Phase 10L started from the clean K4 checkpoint `fc8225d66fe702f0959765f499cc6cf7aebe0166` (`feat: add routine history and pilot hardening`). Phase 10M applies the approved 2026-08-07 operational-standards amendment on the merged source checkpoint `467d2c116ba4e07e411f4b1b21764c0293a4ed3f`. The frozen installed baseline remains `content/routine-engine/mesh-routine-content-v1.json`: `mesh-routine-content@1.1R`, hash `c149a8416a867dcb7d87224f3ae8e2a214e5ca4954613b118521ebe5ae3aff2a`. Phase 10Q adds the separate reviewed manifest `content/routine-engine/mesh-routine-content-v1-2r.json`: `mesh-routine-content@1.2R`, hash `2dcfc69b822f973c23e54934b6799faa5b9400ae0529096f049067811a417f25`. The generator verifies the three unchanged re-locked Opening, Closing and Double Shift source hashes, hashes only the decision bodies of both human amendments (excluding their generated metadata), and applies the layers deterministically. Both manifests have 37 Opening tasks in three sections, 46 Closing tasks in two sections, and four Double Shift system-step definitions plus bundle copy. Double Shift remains a bundle over linked Opening and Closing runs and is never a third routine template.

The migration embeds the generated canonical JSON but performs no top-level install. A personal same-organization manager may call the read-only preview in `legacy` or `shadow` and, after an explicit note, expected organization-state hash and stable idempotency key, invoke one atomic installer. The installer rejects `pilot` and `active`, conflicts, stale state and reused keys with changed requests. It creates or exactly reuses locations, ordered location sets, standards and logical reference placeholders, then creates only editable Opening and Closing drafts. Existing semantic conflicts, drafts and published templates block the initial install and are never overwritten. Installation does not publish, create a run/task snapshot/bundle/delivery, change mode or change release stage. Immutable installation and operation rows record the result. The manager UI says: “This installs editable drafts only. Nothing is published or operational.”

The pack defines 44 locations without room 005, 12 ordered sets, 14 standards and 40 logical reference placeholders. Cup and wine full/service-ready identities now hold semantically identical structured visual layouts rather than artificial counts: all defined positions are filled, ordinary cups are four high with handles right, cappuccino shelf/machine-top and espresso machine-top positions are explicit, and anything still in washing is not ready. O29 and O35 remain independent physical checks. The Workbar contract assigns exactly four Coffee Canisters with a 1 Members lounge + 3 Workbar/kitchen-reserve service distribution and fixed overnight cupboard. Tea names/order, weekday front-door schedule, per-door Salto/physical verification, double locking at Cornerbar street, universal-key fridge behavior, unlocked grille/milk-fridge exceptions and the separate Cornerbar Operating Standard are current structured revisions.

Only `serviceware-office-recovery-route-confirmation` deliberately has no current revision: **Exact serviceware recovery route through the relevant office floors**. No zero, empty string, `TBD`, fake office-floor point or guessed rule satisfies it. Missing actual images remain warnings, not blockers; every logical placeholder retains `Referansebilde kommer`, `Vis hvordan det skal se ut`, and the Phase 10C prepare → server-selected path → upload → finalize workflow for later Manager Control Center uploads.

The locked-source-plus-amendment task model retains every instruction, structured item, done criterion, deviation/blocking rule and reference-guidance block with a deterministic provenance hash. It preserves O13 eggs, independent O29/O35 physical final-restock checks, O27→O29 and O33→O35 continuous completion, Previous Closing evidence as context only, and transfer/evidence handling for event-active work. C17 accounts only for four Workbar-assigned Coffee Canisters; C27 emits layout/accountability evidence without false totals. Cornerbar event-active work is formally transferred by fridge, door/lock, equipment, music/lighting, sweep and reset scope and is never N/A. Cross-run relations are declarative and never auto-complete Opening; delivery comparisons use same-scope typed evidence. The generated audit lists every O/C task’s section, policies, timing, location/set, item/reference/relation counts and provenance hash.

`get_mesh_routine_content_pack_audit()` returns installation and canonical hashes, installed draft IDs/hashes, exact/missing/duplicate O/C IDs, titles, sections, item/reference/dependency/relation counts, DS definitions, the sole unresolved requirement, operation history and semantic divergence. A later manager edit is reported as changed from the installed pack and is not auto-repaired. Release readiness models operational content as two published templates plus four DS system steps; installation remains a draft-only state and the unresolved serviceware route keeps publication/readiness blocked.

Phase 10L adds the lazy isolated `RoutineContentPackManager` under `routines-v2/manager/`; `App.jsx` remains unchanged in 10M. Frontend writes use only the content installer or the existing Phase 10A–10J mutation RPCs. The preview, note and stable idempotency key survive stale, network, auth and validation failures, and no automatic rebase occurs. The generated SQL, human audit, amendment metadata and canonical manifest are compared deterministically, including a fresh rebuild from all three locked sources plus the hashed amendment decision body.

No Supabase production migration, content installation, publication, run/task/bundle/delivery creation, mode/stage/shadow/pilot/active change, Auth change, Stock Count/Inventory Storage change, Event Operations change, legacy write or deployment has been performed by the 10M amendment. The amendment remains an uncommitted working-tree implementation pending its own review. The accepted Phase 9L baseline remains exactly `Phase 9L requires exactly one approved August shelf/storage source session.`
