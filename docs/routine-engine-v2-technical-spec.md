...
`npm run verify:routine-full-migration-reapply` uses network-isolated disposable PostgreSQL 17.6 databases in rehearsal (`session_user=phase10o_rehearsal_login`, `current_user=supabase_admin` through `SET ROLE`) and production-shaped (`session_user=current_user=postgres`) owner contexts. The temporary rehearsal login exists only inside the disposable database and is explicitly made a member of the migration owner. Each context applies all 24 migrations through Phase 10V three times—72/72 applications—with no out-of-band manager bootstrap. Before and after 10O it compares every existing Routine relation/function/schema ACL, policy, function body/config, relation definition, Storage policy, Realtime membership, settings row, Routine row count and protected-domain fingerprint; only `pg_default_acl` may change. The 10V step additionally proves that exactly four legacy creation-key constraints and their owned indexes are absent while all six resource/participant business-identity indexes remain valid. Future table/sequence/function probes must be owned by the same effective role as `pg_default_acl.defaclrole`, expose no client privilege and retain owner access. Negative probes reintroduce each default privilege and must be detected.

The portable semantic schema contract is `routine-canonical-catalog-forensics-v1`. It deparses with `search_path=pg_catalog`, uses schema-qualified identities and deterministic category/identity ordering, replaces only object-owner values with `<OBJECT_OWNER>`, and excludes relation/function/schema/default ACL records. It includes schemas, relation and column semantics, constraints/FK targets, indexes/predicates, types/enums/domains, triggers, full function signature/config/body hashes, RLS/policies, the Storage policy/bucket contract and Routine Realtime membership. After the Phase 10S runtime alignment, Phase 10T participant conflict alignment, Phase 10U operation-ledger convergence, and Phase 10V creation-provenance alignment, its accepted fingerprint is `1285920a3b13f2337871e3355751fc9043f17e380c55dec43f86ca01cff612e5` in both owner contexts. Real disposable column/default, FK, index, policy, function-body, search-path and forced-RLS mutations must all fail this gate.
