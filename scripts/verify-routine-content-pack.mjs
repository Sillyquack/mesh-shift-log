...
const amendmentMigration="supabase/phase10r_mesh_routine_content_pack_1_3r.sql";
const securityMigration="supabase/phase10o_routine_default_privilege_hardening.sql";
const paths={pack:"content/routine-engine/mesh-routine-content-v1-3r.json",previousPack:"content/routine-engine/mesh-routine-content-v1-2r.json",baselinePack:"content/routine-engine/mesh-routine-content-v1.json",generator:"scripts/generate-routine-content-pack.mjs",doc:"docs/routine-engine-v2-mesh-content-v1-3r.md",baseAmendment:"docs/routine-engine-v2-mesh-operational-standards-amendment-2026-08-07.md",productionAmendment:"docs/routine-engine-v2-production-readiness-amendment-2026-08-09.md",amendment:"docs/routine-engine-v2-serviceware-route-amendment-2026-08-09.md",manifest:"src/features/routines-v2/data/routineProductionReadinessAmendmentManifest.js",fixture:"supabase/tests/phase10/content-pack-fixtures.sql",assertions:"supabase/tests/phase10/content-pack-assertions.sql"};
...
function sourceChecks(){
  ...
  const pack=JSON.parse(readFileSync(absolute(paths.pack),"utf8"));const previousPack=JSON.parse(readFileSync(absolute(paths.previousPack),"utf8"));const baselinePack=JSON.parse(readFileSync(absolute(paths.baselinePack),"utf8"));const generator=readFileSync(absolute(paths.generator),"utf8");const baselineSql=readFileSync(absolute(contentMigration),"utf8");const readinessSql=readFileSync(absolute(readinessMigration),"utf8");const sql=readFileSync(absolute(amendmentMigration),"utf8");
  check("full Phase 10 manifest ends with 10L, 10P, 10Q, 10O, then 10R",migrations.length===20&&migrations.at(-5)===contentMigration&&migrations.at(-4)===readinessMigration&&migrations.at(-3).endsWith("phase10q_mesh_routine_content_pack_1_2r.sql")&&migrations.at(-2)===securityMigration&&migrations.at(-1)===amendmentMigration);
  ...
  check("frozen 1.2R predecessor remains exact",previousPack.packVersion==="1.2R"&&previousPack.packHash==="2dcfc69b822f973c23e54934b6799faa5b9400ae0529096f049067811a417f25");
  ...
  check("pack minor version is 1.3R",pack.packVersion==="1.3R"&&pack.packHash==="b416001c2885bbf54bdb029b8e7164cbb903a76b8344396a4e9fcffa26107fe1");
  const packText=JSON.stringify(pack);
  check("1.3R preserves the reviewed Coffee Canisters terminology",(packText.match(/Coffee Canister(?!s)/g)??[]).length===14&&(packText.match(/Coffee Canisters/g)??[]).length===59);
  ...
  check("1.3R SQL payload is generated",sql.includes(`$mesh_content$${JSON.stringify(pack)}$mesh_content$`));
  check("10R is a transaction-bounded provider-only replacement",/^begin;/i.test(sql.trim())&&/commit;\s*$/i.test(sql.trim())&&sql.includes("create or replace function public.routine_mesh_content_pack_v1()")&&sql.includes("revoke all on function public.routine_mesh_content_pack_v1() from public, anon, authenticated"));
  check("10R cannot mutate settings, content, release state, or operative rows",!/^(?!\s*--).*\b(?:insert\s+into|update\s+|delete\s+from|truncate\s+|merge\s+into)\b/im.test(sql)&&!/[\s\S]\bset\s+(?:mode|ui_release_stage)\s*=/i.test(sql));
...
async function main(){
  ...
  psql(readFileSync(absolute(amendmentMigration),"utf8"),{transaction:true});
  check("Phase 10R reapplies to the identical provider definition",scalar("select md5(pg_get_functiondef('public.routine_mesh_content_pack_v1()'::regprocedure));")===amendmentDefinition);
  check("Phase 10R reapply creates no installation or content state",scalar("select md5(coalesce(jsonb_agg(to_jsonb(value) order by value.organization_id,value.pack_key),'[]'::jsonb)::text) from public.routine_content_pack_installations value;")===stateBefore10Q);
