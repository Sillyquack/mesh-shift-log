...
const amendmentMigration="supabase/phase10q_mesh_routine_content_pack_1_2r.sql";
const securityMigration="supabase/phase10o_routine_default_privilege_hardening.sql";
const paths={pack:"content/routine-engine/mesh-routine-content-v1-2r.json",baselinePack:"content/routine-engine/mesh-routine-content-v1.json",generator:"scripts/generate-routine-content-pack.mjs",doc:"docs/routine-engine-v2-mesh-content-v1-2r.md",baseAmendment:"docs/routine-engine-v2-mesh-operational-standards-amendment-2026-08-07.md",amendment:"docs/routine-engine-v2-production-readiness-amendment-2026-08-09.md",fixture:"supabase/tests/phase10/content-pack-fixtures.sql",assertions:"supabase/tests/phase10/content-pack-assertions.sql"};
...
function sourceChecks(){
  ...
  const pack=JSON.parse(readFileSync(absolute(paths.pack),"utf8"));const baselinePack=JSON.parse(readFileSync(absolute(paths.baselinePack),"utf8"));const generator=readFileSync(absolute(paths.generator),"utf8");const baselineSql=readFileSync(absolute(contentMigration),"utf8");const readinessSql=readFileSync(absolute(readinessMigration),"utf8");const sql=readFileSync(absolute(amendmentMigration),"utf8");
  check("full Phase 10 manifest ends with 10L, 10P, 10Q, then 10O",migrations.length===19&&migrations.at(-4)===contentMigration&&migrations.at(-3)===readinessMigration&&migrations.at(-2)===amendmentMigration&&migrations.at(-1)===securityMigration);
  ...
  check("only the serviceware office route remains unresolved",pack.unresolvedRequirements.length===1&&pack.unresolvedRequirements[0].standardKey==="serviceware-office-recovery-route-confirmation"&&!Object.hasOwn(standards["serviceware-office-recovery-route-confirmation"],"currentRevision"));
  ...
  check("1.2R SQL payload is generated",sql.includes(`$mesh_content$${JSON.stringify(pack)}$mesh_content$`));
  check("10Q is a transaction-bounded provider-only replacement",/^begin;/i.test(sql.trim())&&/commit;\s*$/i.test(sql.trim())&&sql.includes("create or replace function public.routine_mesh_content_pack_v1()")&&sql.includes("revoke all on function public.routine_mesh_content_pack_v1() from public, anon, authenticated"));
  check("10Q cannot mutate settings, content, release state, or operative rows",!/^(?!\s*--).*\b(?:insert\s+into|update\s+|delete\s+from|truncate\s+|merge\s+into)\b/im.test(sql)&&!/[\s\S]\bset\s+(?:mode|ui_release_stage)\s*=/i.test(sql));
  ...
  check("inventory source is read-only",JSON.stringify(pack).includes('"locationCode":"WORKBAR_NON_ALCO_FRIDGE"')&&!/(insert|update|delete)[\s\S]{0,100}inventory_/i.test(`${baselineSql}\n${sql}`));
...
async function main(){
  ...
  check("Phase 10Q exposes the exact 1.2R provider",scalar("select (public.routine_mesh_content_pack_v1()->>'packVersion')||':'||(public.routine_mesh_content_pack_v1()->>'packHash');")==="1.2R:2dcfc69b822f973c23e54934b6799faa5b9400ae0529096f049067811a417f25");
  const amendmentDefinition=scalar("select md5(pg_get_functiondef('public.routine_mesh_content_pack_v1()'::regprocedure));");
  psql(readFileSync(absolute(amendmentMigration),"utf8"),{transaction:true});
  check("Phase 10Q reapplies to the identical provider definition",scalar("select md5(pg_get_functiondef('public.routine_mesh_content_pack_v1()'::regprocedure));")===amendmentDefinition);
  check("Phase 10Q reapply creates no installation or content state",scalar("select md5(coalesce(jsonb_agg(to_jsonb(value) order by value.organization_id,value.pack_key),'[]'::jsonb)::text) from public.routine_content_pack_installations value;")===stateBefore10Q);
  ...
  const standardsReadiness=JSON.parse(scalar("select public.routine_compute_pilot_readiness('c3000000-0000-4000-8000-000000000001')::text;"));
