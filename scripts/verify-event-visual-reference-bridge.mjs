...
function command(name, args, options = {}) {
  const response = spawnSync(name, args, {
    cwd: ROOT,
    encoding: 'utf8',
    input: options.input,
...
function scalar(sql) {
  const lines = psql(sql, { tuplesOnly: true }).stdout
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  return lines.at(-1) || '';
...
function staticVerification() {
  ...
  check('isolated helper allows only manager and Event Floor Manager', /profile\.role in \('manager', 'event_floor_manager'\)/.test(migration));
  check('inactive, shared, and organization-less profiles are denied', /profile\.active = true[\s\S]*?organization_id is not null[\s\S]*?is_shared_device, false\) = false/.test(migration));
  check('unsupported keys fail before lookup', /unsupported key/.test(migration));
  ...
  check('object paths leave the RPC only for active images', /case when row\.state = 'active_image' then row\.object_path else null end/.test(migration));
...
function expectFailure(label, sql, pattern) {
  ...
  check(label, response.status !== 0 && pattern.test(`${response.stdout}\n${response.stderr}`));
}

function databaseVerification() {
  const users = {
    eventA: '33000000-0000-4000-8000-000000000001',
    eventB: '33000000-0000-4000-8000-000000000002',
    inactive: '33000000-0000-4000-8000-000000000003',
    shared: '33000000-0000-4000-8000-000000000004',
    managerA: '11000000-0000-4000-8000-000000000001',
    staffA: '11000000-0000-4000-8000-000000000002',
  };
  const paths = {
    currentA: 'a1000000-0000-4000-8000-000000000001/e1000000-0000-4000-8000-000000000001/e2000000-0000-4000-8000-000000000002/atrium-cafe.jpg',
    oldA: 'a1000000-0000-4000-8000-000000000001/e1000000-0000-4000-8000-000000000001/e2000000-0000-4000-8000-000000000001/atrium-cafe-old.jpg',
    privateA: 'a1000000-0000-4000-8000-000000000001/e1000000-0000-4000-8000-000000000003/e2000000-0000-4000-8000-000000000004/private-manager-only.jpg',
    currentB: 'b2000000-0000-4000-8000-000000000001/e1000000-0000-4000-8000-000000000004/e2000000-0000-4000-8000-000000000005/atrium-cafe.jpg',
  };

  const payload = JSON.parse(scalar(authenticatedSql(users.eventA, `
    select public.get_event_visual_references(array['atrium-cafe', 'atrium-water'])::text;
  `)));
  check('Event Floor Manager receives every requested allowed key', payload.references.length === 2);
  const cafe = payload.references.find((row) => row.referenceKey === 'atrium-cafe');
  const water = payload.references.find((row) => row.referenceKey === 'atrium-water');
  check('Event Floor Manager receives only current active café image', cafe?.state === 'active_image' && cafe?.objectPath === paths.currentA);
  check('placeholder metadata has no object path', water?.state === 'placeholder' && water?.objectPath === null);
  check('sanitized payload omits organization and audit identities', !/organization|authUser|createdBy|updatedBy/i.test(JSON.stringify(payload)));

  const crossPayload = JSON.parse(scalar(authenticatedSql(users.eventB, `
    select public.get_event_visual_references(array['atrium-cafe'])::text;
  `)));
  check('same key resolves only inside caller organization', crossPayload.references[0]?.objectPath === paths.currentB);

  for (const [label, userId] of [
    ['staff', users.staffA],
    ['inactive Event Floor Manager', users.inactive],
    ['shared Event Floor Manager', users.shared],
  ]) {
    expectFailure(
      `${label} cannot call Event visual metadata RPC`,
      authenticatedSql(userId, `select public.get_event_visual_references(array['atrium-cafe']);`),
      /Event visual-reference access requires/i,
    );
  }
  expectFailure(
    'unsupported visual keys are rejected',
    authenticatedSql(users.eventA, `select public.get_event_visual_references(array['private-manager-only']);`),
    /unsupported key/i,
  );

  const helperRead = (userId, path) => scalar(authenticatedSql(userId, `
    select public.routine_reference_storage_can_read('${path}')::text;
  `));
  check('Event Floor Manager can read current allowed object', helperRead(users.eventA, paths.currentA) === 'true');
  check('Event Floor Manager cannot read old version', helperRead(users.eventA, paths.oldA) === 'false');
  check('Event Floor Manager cannot read manager-only reference', helperRead(users.eventA, paths.privateA) === 'false');
  check('Event Floor Manager cannot read another organization', helperRead(users.eventA, paths.currentB) === 'false');

  const storageCount = (userId) => scalar(authenticatedSql(userId, `
    select count(*)::text from storage.objects where bucket_id = 'routine-reference-images';
  `));
  check('manager keeps current, old, and manager-only same-org Storage access', storageCount(users.managerA) === '3');
  check('Event Floor Manager Storage RLS exposes one current allowed object', storageCount(users.eventA) === '1');
  check('ordinary staff receives no unlinked event objects', storageCount(users.staffA) === '0');
  check('Event Floor Manager table RLS exposes zero reference rows', scalar(authenticatedSql(users.eventA, `
    select count(*)::text from public.routine_reference_images;
  `)) === '0');
  check('internal Event helpers are not executable by authenticated', scalar(`
    select (
  ...
  `) === 'true');
  check('sanitized metadata and Storage predicates are authenticated boundaries', scalar(`
    select (
...
async function main() {
  ...
  docker([
    ...
    '--network', 'none', '--env', `POSTGRES_PASSWORD=${PASSWORD}`,
  ...
  `);
  for (const path of BASELINES) psql(readFileSync(absolute(path), 'utf8'), { transaction: true });
  psql(`
