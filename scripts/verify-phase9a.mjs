...
const checks = [
  ...
  check('MIG-11: Phase 9G-C is the sole repeatable terminal layer after retained Phase 9F through 9G-B2 coverage', migrationEntries.filter((entry) => entry.repeatable).length === 1 && migrationEntries.at(-1).path === 'supabase/phase9gc_inventory_counter_mobile.sql' && migrationEntries.at(-2).path === 'supabase/phase9gb2_inventory_counter_replacement.sql' && migrationEntries.at(-3).path === 'supabase/phase9gb_inventory_counter_assignments.sql' && migrationEntries.at(-4).path === 'supabase/phase9g_inventory_operational_scope.sql' && /inventory_count_line_structured_quantity/i.test(phase9fSql) && /inventory_refrigerator_reserve_targets/i.test(phase9gSql) && /three 0\.7 L sealed bottles/i.test(structuredQuantityVerification)),
