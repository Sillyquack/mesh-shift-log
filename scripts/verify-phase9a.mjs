...
const phase9kSql = readFileSync(new URL('../supabase/phase9k_millum_complete_count_export.sql', import.meta.url), 'utf8');
const phase9lSql = readFileSync(new URL('../supabase/20260804123921_phase9l_millum_august_carry_forward_and_future_scope.sql', import.meta.url), 'utf8');
const phase9mSql = readFileSync(new URL('../supabase/20260804151500_phase9m_millum_snapshot_supplement.sql', import.meta.url), 'utf8');
const phase9nSql = readFileSync(new URL('../supabase/20260804180000_phase9n_millum_single_authoritative_session.sql', import.meta.url), 'utf8');
const phase9oSql = readFileSync(new URL('../supabase/20260804200000_phase9o_millum_wine_value_conversion.sql', import.meta.url), 'utf8');
const phase9pSql = readFileSync(new URL('../supabase/20260805035957_phase9p_millum_export_explanations.sql', import.meta.url), 'utf8');
...
const checks = [
  ...
  check('20a: session selector no longer expands parent areas into the count scope', !/effectiveInventoryLocationIds/i.test(workspace) && /<legend>Locations included<\/legend>/i.test(workspace)),
  ...
  check('MIG-11: repeatable Phase 9H-9K precede one-time Phase 9L-9P export hardening', JSON.stringify(migrationEntries.filter((entry) => entry.repeatable).map((entry) => entry.path)) === JSON.stringify(['supabase/phase9h_inventory_session_location_scope.sql', 'supabase/phase9i_millum_stock_count_exports.sql', 'supabase/phase9j_inventory_shelf_storage_guidance.sql', 'supabase/phase9k_millum_complete_count_export.sql']) && migrationEntries.at(-1).path === 'supabase/20260805035957_phase9p_millum_export_explanations.sql' && migrationEntries.at(-2).path === 'supabase/20260804200000_phase9o_millum_wine_value_conversion.sql' && migrationEntries.at(-3).path === 'supabase/20260804180000_phase9n_millum_single_authoritative_session.sql' && migrationEntries.at(-4).path === 'supabase/20260804151500_phase9m_millum_snapshot_supplement.sql' && migrationEntries.at(-5).path === 'supabase/20260804123921_phase9l_millum_august_carry_forward_and_future_scope.sql' && migrationEntries.at(-6).path === 'supabase/phase9k_millum_complete_count_export.sql' && migrationEntries.at(-7).path === 'supabase/phase9j_inventory_shelf_storage_guidance.sql' && migrationEntries.at(-8).path === 'supabase/phase9i_millum_stock_count_exports.sql' && migrationEntries.at(-9).path === 'supabase/phase9h_inventory_session_location_scope.sql' && /inventory_count_line_structured_quantity/i.test(phase9fSql) && /inventory_refrigerator_reserve_targets/i.test(phase9gSql) && /physical_count_only/i.test(phase9jSql) && /inventory_install_millum_profile_v2/i.test(phase9kSql) && /inventory_millum_export_session_values/i.test(phase9lSql) && /supplementVersion/i.test(phase9mSql) && /sourceRuleVersion/i.test(phase9nSql) && /where line\.session_id = v_session\.id/i.test(phase9nSql) && /wineValueRuleVersion/i.test(phase9oSql) && /exportAuditRuleVersion/i.test(phase9pSql) && /three 0\.7 L sealed bottles/i.test(structuredQuantityVerification)),
