...
const checks = [
  ...
  check('MIG-3: Phase 9G is terminal and unsafe older-phase reapplication is rejected', migrationPaths.at(-1) === PHASE9_TERMINAL_MIGRATION && unsafeMigrationOrderRejected),
  ...
  check('MIG-11: Phase 9G is the sole repeatable terminal layer after retained Phase 9F coverage', migrationEntries.filter((entry) => entry.repeatable).length === 1 && migrationEntries.at(-1).path === 'supabase/phase9g_inventory_operational_scope.sql' && /inventory_count_line_structured_quantity/i.test(phase9fSql) && /inventory_refrigerator_reserve_targets/i.test(phase9gSql) && /three 0\.7 L sealed bottles/i.test(structuredQuantityVerification)),
